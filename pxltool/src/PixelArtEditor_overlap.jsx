...
const PixelArtEditor = () => {
    ...
    const drawBridgeFillet = (ctx, b, r, isSvg = false) => {
        ...
        const oX = (overlap / 2) * visualPixelWidth;
        const oY = (overlap / 2) * visualPixelHeight;

        let cx = b.X * visualPixelWidth;
        let cy = b.Y * visualPixelHeight;

        // Shift fillet anchor based on quadrant and overlap
        // A bridge fillet sits in a concave corner formed by two pixels.
        // We shift the anchor to the intersection of the shifted edges.
    ...
    const drawScene = useCallback((ctx, isExport = false) => {
        ...
        if (overlap < 0) {
            // Draw individual pixels for natural disconnection
            pixels.forEach(key => {
                const [x, y] = key.split(',').map(Number);
                const rect = {
                    x1: x - overlap / 2,
            ...
            contours.forEach((contour, contourId) => {
                ...
                const getAdjusted = (v, sIn, sOut) => {
                    let dx = 0, dy = 0;
                    // Use a Set to ensure each side's offset is only applied once (avoids double-shifting collinear vertices)
                    const sides = new Set([sIn, sOut]);
                    sides.forEach(s => {
                        if (s === 'top') dy -= overlap / 2;
                        if (s === 'bottom') dy += overlap / 2;
                        if (s === 'left') dx -= overlap / 2;
                        if (s === 'right') dx += overlap / 2;
                    });
                    return { x: (v.x + dx) * visualPixelWidth, y: (v.y + dy) * visualPixelHeight };
                };

                const lastEdge = contour[contour.length - 1];
                const startV = getAdjusted({ x: contour[0].x1, y: contour[0].y1 }, lastEdge.side, contour[0].side);
                ctx.moveTo(startV.x, startV.y);

                contour.forEach((edge, i) => {
                    const nextEdge = contour[(i + 1) % contour.length];
                    const vertex = getAdjusted({ x: edge.x2, y: edge.y2 }, edge.side, nextEdge.side);

                    const px = vertex.x;
                    const py = vertex.y;

                    const vertexGridX = edge.x2;
                    const vertexGridY = edge.y2;

                    let shouldRound = roundedCorners.has(`${contourId}-${i}`);
                    if (bridgeMap.has(`${vertexGridX},${vertexGridY}`)) shouldRound = false;
        ...
        ctx.fill();
    }, [gridWidth, gridHeight, pixels, findContour, roundness, overlap, roundedCorners, bridgeMap, bridges, getDisplayMetrics]);
    ...
    const getSVGString = () => {
        ...
        if (overlap < 0) {
            ...
            pixels.forEach(key => {
                ...
                const x2 = rect.x2 * visualPixelWidth;
                const y2 = rect.y2 * visualPixelHeight;
                // Maximum radius is half the shortest dimension
                const rectW = x2 - x1;
                const rectH = y2 - y1;
                const rad = Math.min(r, rectW * 0.5, rectH * 0.5);
                ...
                <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-lg">
                    ...
                    <div className="mt-4 pt-4 border-t border-gray-100">
                        <div className="flex justify-between items-center mb-1">
                            ...
                            <span className="text-xs font-mono bg-gray-100 px-2 py-1 rounded text-gray-600">{overlap.toFixed(2)}</span>
                        </div>
                        <input type="range" min="-1" max="1" step="0.05" value={overlap} onChange={(e) => setOverlap(parseFloat(e.target.value))} className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-black" />
                    </div>
                </div>
