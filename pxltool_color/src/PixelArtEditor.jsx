                ...
                <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-lg">
                    ...
                    <div className="mb-4">
                        <div className="grid grid-cols-5 gap-2 mb-4">
                            {palette.map(c => (
                                <button key={c} onClick={() => setCurrentColor(c)} style={{ backgroundColor: c }} className={`w-full aspect-square rounded-lg border border-gray-100 transition hover:scale-105 ${currentColor === c ? 'ring-2 ring-black ring-offset-1' : ''}`} />
                            ))}

                            <div className="relative w-full aspect-square rounded-lg border border-gray-200 bg-white flex items-center justify-center overflow-hidden transition hover:bg-gray-50 group">
                                <div className="absolute inset-0" style={{ backgroundColor: currentColor }} />
                                <Plus size={16} className="text-white mix-blend-difference z-10" />
                                <input type="color" value={currentColor} onChange={(e) => setCurrentColor(e.target.value)} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" title="Custom Color" />
                            </div>

                            <button onClick={() => setTool('eyedropper')} className={`w-full aspect-square rounded-lg border flex items-center justify-center transition ${tool === 'eyedropper' ? 'bg-black text-white' : 'bg-white text-gray-600 hover:bg-gray-100'}`} title="Eyedropper"><Droplet size={16} /></button>
                        </div>
